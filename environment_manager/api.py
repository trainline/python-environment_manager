...
class EMApi(object):
    ...
    def get(self, query_endpoint=None, retries=5, backoff=2):
    ...
    def post(self, query_endpoint=None, retries=5, backoff=2):
        """ Function to querying Environment Manager """
        log = LogWrapper()
        if query_endpoint is None:
            log.info('No query endpoint specified, cant just go and query nothing')
    ...
    def delete(self, query_endpoint=None, retries=5, backoff=2):
        """ Function to querying Environment Manager """
        log = LogWrapper()
        if query_endpoint is None:
            log.info('No query endpoint specified, cant just go and query nothing')
    ...
    def put(self, query_endpoint=None, retries=5, backoff=2):
        """ Function to querying Environment Manager """
        log = LogWrapper()
        if query_endpoint is None:
            log.info('No query endpoint specified, cant just go and query nothing')
    ...
    def get_accounts_config(self, **kwargs):
        ...
        return self.get(request_endpoint, **kwargs)
